...
class Dataset(object):
        ...
        def read_jsonl(f):
            ...
            for line in f:
                ...
                if k == "sentence":
                    ...
                    o["tokens"] = o["orig_tokens"] = o["tree_str"] = o["actions"] = o[
                        "in_order_actions"
                    ] = o["tags"] = None
                    sents.append(o)
                else:
    ...
    def _get_grouped_len_to_idxs(
        self, sent_idxs=[], use_action_len=False, max_length_diff=20
    ):
        if use_action_len:
        ...
        sent_idxs_with_len = sorted(
            [(i, get_length(self.sents[i])) for i in sent_idxs], key=lambda x: x[1]
        )
        b = 0
        last_idx = 0
        while b < len(sent_idxs_with_len):
            ...
            max_len = sent_idxs_with_len[
                min(b + group_size, len(sent_idxs_with_len) - 1)
            ][1]
            if (
                max_len - min_len < max_length_diff
            ):  # small difference in a group -> regist as a group
                group = [i for i, l in sent_idxs_with_len[b : b + group_size]]
                b += group_size
            else:
                e = b + 1
                while (
                    e < len(sent_idxs_with_len)
                    and sent_idxs_with_len[e][1] - min_len < max_length_diff
                ):
                    e += 1
