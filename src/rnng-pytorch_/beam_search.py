# This code was provided by Ryo Yoshida.
...
parser.add_argument(
    "--particle_filter", action="store_true", help="search with particle filter"
)
...
parser.add_argument("--top_n_lls_output_file", type=str)
...
parser.add_argument("--tree_output_file", type=str)
...
def main(args):
    ...
    if args.top_n_lls_output_file:
        return_top_n_lls = True
    else:
    ...
    def sort_and_print_trees(block_idxs, block_parses, block_surprisals):
        ...
        orig_order_parses = [
            block_parses[parse_idx] for (parse_idx, _) in parse_idx_to_sent_idx
        ]
        orig_order_surps = [
            block_surprisals[parse_idx] for (parse_idx, _) in parse_idx_to_sent_idx
        ]
        ...
        if args.tree_output_file:
            with open(args.tree_output_file, "wt") as f:
        ...
        for pointer, bucket_i, beam, word_completed in beam_history:
            ...
            for batch_i in range(len(beam)):
                for b in beam[batch_i]:
                    print(
                        "beam: b_i: {}, {}".format(
                            batch_i, b.dump(action_dict, sents[batch_i])
                        )
                    )
                for b in word_completed[batch_i]:
                    print(
                        "word_comp: b_i: {}, {}".format(
                            batch_i, b.dump(action_dict, sents[batch_i])
                        )
                    )
                print()
    ...
    if args.particle_filter:
        ...
        def parse(
            tokens, subword_end_mask, return_beam_history=False, stack_size_bound=-1
        ):
        ...
        def parse(
            ...
            return model.word_sync_beam_search(
                ...
                stack_size_bound=stack_size_bound,
                return_top_n_lls=return_top_n_lls,
            )
    ...
    def try_parse(tokens, subword_end_mask, stack_size_bound=None):
        ...
        if args.dump_beam:
            parses, surprisals, beam_history = parse(
                tokens, subword_end_mask, True, stack_size_bound, False
            )
            dump_histroy(beam_history, [dataset.sents[idx] for idx in batch_idx])
            top_n_lls = None
        elif return_top_n_lls:
            parses, surprisals, top_n_lls = parse(
                tokens, subword_end_mask, False, stack_size_bound, True
            )
            beam_history = None
        else:
            parses, surprisals = parse(
                tokens, subword_end_mask, False, stack_size_bound
            )
            beam_history = None
            top_n_lls = None
        return parses, surprisals, beam_history, top_n_lls
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(batches):
            ...
            parses, surprisals, beam_history, top_n_lls = try_parse(
                tokens, subword_end_mask
            )
            if any(len(p) == 0 for p in parses):
                # parse failure (on some tree in a batch)
                failed_sents = [
                    (idx, " ".join(dataset.sents[idx].orig_tokens))
                    for p, idx in zip(parses, batch_idx)
                    if len(p) == 0
                ]
                failed_sents_str = "\n".join(
                    "{}: {}".format(i, s) for i, s in failed_sents
                )
                logger.warning(
                    "Parse failure occurs on the following "
                ...
                if args.stack_size_bound < tokens.size(1):
                    ...
                    logger.warning(
                        "Current sack size bound {} is smaller than "
                        "sentence length {}.".format(
                            args.stack_size_bound, tokens.size(1)
                        )
                    )
                    logger.warning("Try to reparse with increased stack size bound...")
                    parses, surprisals, beam_history, top_n_lls = try_parse(
                        tokens, subword_end_mask, stack_size_bound=-1
                    )
            ...
            if return_top_n_lls:
                block_top_n_lls.extend(top_n_lls)
            ...
            if cur_block_size >= args.block_size:
                ...
                parse_idx_to_sent_idx = sorted(
                    list(enumerate(block_idxs)), key=lambda x: x[1]
                )
                if return_top_n_lls:
                    orig_order_lls = [
                        block_top_n_lls[parse_idx]
                        for (parse_idx, _) in parse_idx_to_sent_idx
                    ]
                    all_top_n_lls.extend(orig_order_lls)
    ...
    if return_top_n_lls:
        orig_order_lls = [
            block_top_n_lls[parse_idx] for (parse_idx, _) in parse_idx_to_sent_idx
        ]
        all_top_n_lls.extend(orig_order_lls)
    sort_and_print_trees(block_idxs, block_parses, block_surprisals)
    end_time = time.time()
    ...
    if args.top_n_lls_output_file:
        with open(args.top_n_lls_output_file, "wt") as o:
            for sent_i, (sent, top_n_lls) in enumerate(
                zip(dataset.sents, all_top_n_lls)
            ):
                orig_tokens = sent.orig_tokens
                input_tokens = [vocab.id_to_word(t_id) for t_id in sent.token_ids]
                subword_spans = sent.get_subword_span_index()
                piece_top_n_lls = [
                    [top_n_lls[j] for j in span] for span in subword_spans
                ]
                pieces = [[input_tokens[j] for j in span] for span in subword_spans]
                assert len(orig_tokens) == len(piece_top_n_lls)

                for t_i in range(len(orig_tokens)):
                    orig_t = orig_tokens[t_i]
                    for s_i, (piece, piece_top_n_ll) in enumerate(
                        zip(pieces[t_i], piece_top_n_lls[t_i])
                    ):
                        piece_top_n_ll_str = " ".join([str(x) for x in piece_top_n_ll])
                        o.write(
                            "{}\t{}\t{}\t{}\t{}\t{}\n".format(
                                sent_i, t_i, s_i, orig_t, piece, piece_top_n_ll_str
                            )
                        )
