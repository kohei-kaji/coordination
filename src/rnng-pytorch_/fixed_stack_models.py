...
class MultiLayerLSTMCell(nn.Module):
    ...
    def forward(self, input, prev):
        ...
        if prev is None:
            prev = (
                input.new(
                    input.size(0), self.lstm[0].hidden_size, self.num_layers
                ).fill_(0),
                input.new(
                    input.size(0), self.lstm[0].hidden_size, self.num_layers
                ).fill_(0),
        ...
        for i in range(self.num_layers):
            ...
            if i == 0:
                next_hidden_i, next_cell_i = self.lstm[i](
                    input, (prev_hidden_i, prev_cell_i)
                )
            else:
                input_im1 = self.dropout_layer(input_im1)
                next_hidden_i, next_cell_i = self.lstm[i](
                    input_im1, (prev_hidden_i, prev_cell_i)
                )
...
class AttentionComposition(nn.Module):
    def __init__(self, w_dim, dropout, num_labels=10):
        ...
        self.nt_emb = nn.Embedding(
            num_labels, w_dim
        )  # o_nt in the Kuncoro et al. (2017)
    ...
    def forward(
        self, children, ch_lengths, nt, nt_id, stack_state
    ):  # children: (batch_size, n_children, w_dim)

        packed = pack_padded_sequence(
            children, ch_lengths.int().cpu(), batch_first=True, enforce_sorted=False
        )
        h, _ = self.rnn(packed)
        h, _ = pad_packed_sequence(h, batch_first=True)  # (batch, n_children, 2*w_dim)

        rhs = torch.cat(
            [self.nt_emb(nt_id), stack_state], dim=1
        )  # (batch_size, w_dim*2, 1)
        logit = (self.V(h) * rhs.unsqueeze(1)).sum(
            -1
        )  # equivalent to bmm(self.V(h), rhs.unsqueeze(-1)).squeeze(-1)
        len_mask = (
            ch_lengths.new_zeros(children.size(0), 1)
            + torch.arange(children.size(1), device=children.device)
    ...
    def expand_at_dim(self, target_dim, new_size):
        ...
        for a in self.attrs:
            ...
            setattr(
                ...
                a,
                torch.cat(
                    (old_x, old_x.new_zeros(same_dim_except_target(old_x.size()))),
                    target_dim,
                ),
            )
...
class FixedStack(ExpandableStorage):
    def __init__(self, initial_hidden, stack_size, input_size, beam_size=1):
        ...
        self.trees = initial_hidden[0].new_zeros(
            batch_size + (stack_size, input_size), device=device
        )
    ...
    def do_nt(self, nt_batches, nt_embs, nt_ids):
        ...
        self.nt_index[nt_batches + (self.nt_index_pos[nt_batches],)] = (
            self.top_position[nt_batches]
        )

    def do_reduce(self, reduce_batches, new_child):
        prev_nt_position = self.nt_index[
            reduce_batches + (self.nt_index_pos[reduce_batches],)
        ]
    ...
    def collect_reduced_children(self, reduce_batches):
        ...
        child_idx = prev_nt_position.unsqueeze(1) + torch.arange(
            max_ch_length, device=prev_nt_position.device
        )
...
class StackState(ExpandableStorage):
    def __init__(self, batch_size, beam_size, device):
        ...
        self.ncons_nts = torch.zeros(
            (batch_size, beam_size), dtype=torch.long, device=device
        )
        self.nopen_parens = torch.zeros(
            (batch_size, beam_size), dtype=torch.long, device=device
        )
...
class ActionPath(ExpandableStorage):
    ...
    def nbest_parses(self, beam_widths, gen_ll, tgt_batch=None):
        ...
        def tree_actions(batch, beam):
            return (
                actions[batch, beam, 1 : actions_pos[batch, beam] + 1].tolist(),
...
class ParticleAdditionalScores(ExpandableStorage):
    def __init__(self, batch_size, beam_size, initial_K, device):
        ...
        self.K = (
            torch.tensor([initial_K], dtype=torch.float, device=device)
            .expand(batch_size, beam_size)
            .clone()
        )
        self.disc_ll[:, 0] = 0
...
class BeamItems(ExpandableStorage):
    ...
    def marginal_probs(self):
        ...
        return self.gen_ll, torch.logsumexp(self.gen_ll, 1)
    ...
    def _do_move_elements(
        ...
        self.additional.move_beams(
            self_idxs, source.additional, source_idxs, new_additional
        )
...
class RNNGCell(nn.Module):
    ...
    def __init__(
        ...
        self.nt_emb = nn.Sequential(
            nn.Embedding(action_dict.num_nts(), input_size), self.dropout
        )
        self.stack_rnn = MultiLayerLSTMCell(
        ...
        self.composition = (
            ...
            else LSTMComposition(input_size, dropout)
        )
    ...
    def forward(self, word_vecs, actions, stack, subword_end_mask=None):
        ...
        reduce_batches = (actions == self.action_dict.a2i["REDUCE"]).nonzero(
            as_tuple=True
        )
        nt_batches = (actions >= self.action_dict.nt_begin_id()).nonzero(as_tuple=True)
        shift_batches = (actions == self.action_dict.a2i["SHIFT"]).nonzero(
            as_tuple=True
        )
        ...
        if reduce_batches[0].size(0) > 0:
            children, ch_lengths, reduced_nt, reduced_nt_ids = (
            ...
            new_child, _, _ = self.composition(
                children, ch_lengths, reduced_nt, reduced_nt_ids, stack_h
            )
        ...
        new_hidden, new_cell = self.stack_rnn(
            new_input.view(-1, self.input_size),
...
class FixedStackRNNG(nn.Module):
    def __init__(
        ...
        self.rnng = RNNGCell(
            w_dim, h_dim, num_layers, dropout, self.action_dict, attention_composition
        )
    ...
    def word_sync_beam_search(
        ...
        _top_n_lls = [[] for _ in range(x.size(0))]
        top_n_lls = [[] for _ in range(x.size(0))]
        ...
        for pointer in range(x.size(1) + 1):
            ...
            while not finished_batches.all():
                ...
                finished_batches = word_finished_batches()
                beam.beam_widths[finished_batches.nonzero(as_tuple=True)] = (
            ...
            _top_n_ll, marginal = beam.marginal_probs()
            for b, s in enumerate(_top_n_ll.cpu().detach().numpy()):
                _top_n_lls[b].append(s[:word_beam_size])
            ...
            for b, length in enumerate(sent_lengths.cpu().detach()):
                if length == pointer:  # finished
                    for i in range(0, length):
                        top_n_lls[b].append(_top_n_lls[b][i])
                        surprisals[b].append(
                            -word_marginal_ll[b][i]
                            - (-word_marginal_ll[b][i - 1] if i > 0 else 0)
                        )
    ...
    def variable_beam_step(
        ...
        actions = successors[1].new_full(
            (x.size(0), beam.beam_size), self.action_dict.padding_idx
        )
        actions[new_beam_idxs] = successors[1]
    ...
    def new_beam_stack_with_state(self, initial_hidden, stack_size, beam_size):
        ...
        stack_state = StackState(
            initial_hidden[0].size(0), beam_size, initial_hidden[0].device
        )
        return stack, stack_state

    def variable_beam_search(
        self, x, subword_end_mask, K, original_reweight=False, stack_size_bound=100
    ):
        self.eval()
        sent_lengths = (x != self.padding_idx).sum(dim=1)
        ...
        for pointer in range(x.size(1) + 1):
            ...
            _, marginal = beam.marginal_probs()
            for b, s in enumerate(marginal.cpu().detach().numpy()):
                word_marginal_ll[b].append(s)
    ...
    def initial_states(self, x):
        ...
        return [
            TopDownState.from_initial_stack(
                (initial_hidden[0][b], initial_hidden[1][b])
            )
    ...
    def get_successors(
        self, x, subword_end_mask, sent_lengths, pointer, beam, beam_size, shift_size
    ):
        if pointer < x.size(1):
            next_x = x[:, pointer]
        else:
        ...
        return self.scores_to_successors(
            x, sent_lengths, pointer, beam, log_probs, beam_size, shift_size
        )

    def get_successors_by_particle_filter(
        self, x, subword_end_mask, sent_lengths, pointer, beam
    ):
        if pointer < x.size(1):
            next_x = x[:, pointer]
        else:
        ...
        idx = (
            torch.arange(0, new_K.size(1), device=new_K.device)
            .unsqueeze(0)
            .expand(new_K.size())
        )
        ...
        return (
            mask_to_successors(no_end_action_mask),
    ...
    def finalize_particle_beam(
        ...
        self.reweight_and_filter_particles(
            beam, word_completed_beam, K, original_reweight
        )
    ...
    def reweight_and_filter_particles(
        ...
        new_Ks = (
            (log_weights - denom.unsqueeze(-1)).exp_() * K
        ).round_()  # (batch, beam)
    ...
    def action_log_probs(
        self, stack, invalid_action_mask, next_x=None, return_disc_probs=False
    ):
        """
    ...
    def invalid_action_mask(self, beam, sent_lengths, subword_end_mask):
        ...
        except_reduce_mask = (action_order != self.action_dict.a2i["REDUCE"]).view(
            1, 1, -1
        )
        ...
        finished_mask = (
            (beam.stack.pointer == sent_lengths) * (beam.nopen_parens == 0)
        ).unsqueeze(-1)
        beam_width_mask = (
    ...
    def scores_to_successors(
        ...
        within_beam_num_end_actions = bincount_and_supply(
            within_beam_end_action_idx[0], x.size(0)
        )  # (batch_size)
        ...
        outside_num_end_actions = bincount_and_supply(
        ...
        num_forced_completions = bincount_and_supply(
            outside_end_action_idx[0], x.size(0)
        )
